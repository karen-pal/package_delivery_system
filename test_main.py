...
def test_deliver_package():
    ...
    assert (
        AA.deliveries_log[datetime.today().strftime("%Y-%m-%d")]["total_revenue"] == 10
    )
    assert AA.deliveries_log[datetime.today().strftime("%Y-%m-%d")]["deliveries"] == [
        delivery_a
    ]
    ...
    assert (
        AA.deliveries_log[datetime.today().strftime("%Y-%m-%d")]["total_revenue"] == 20
    )
    assert AA.deliveries_log[datetime.today().strftime("%Y-%m-%d")]["deliveries"] == [
        delivery_a,
        delivery_b,
...
def test_transaction_report_by_date():
    ...
    assert (
        report["{}".format(datetime.today().strftime("%Y-%m-%d"))]["total_revenue"]
        == 20
    )
